...
class Paciente:
    ...
    def set_telefone(self, t):
        if t == "": raise ValueError("O telefone não pode ser vazio")
        self.__telefone = t
    ...
    def idade(self):
        tempo = datetime.datetime.now() - self.__nascimento # timedelta é contado em dias
        anos = tempo.days // 365
        meses = tempo.days % 365 // 30
        return f"idade: {anos} ano(s) e {meses} mes(es)"
    
# x = Paciente(1, "Eduardo", "001.002.003-45", "84-90009-1234", datetime.datetime(2010, 1, 25))
# print(x)
# print(x.idade())
...
class PacienteUI:
    ...
    def atualizar(cls):
        ...
        for x in cls.__pacientes:
            ...
            nasc = datetime.datetime.strptime(input("Informe o novo=a data de nascimento: "), "%d/%m/%Y")
            x.set_nome(nome)
            x.set_cpf(cpf)
            x.set_telefone(fone)
